...
from cv2 import (COLOR_BGR2LAB, GC_INIT_WITH_RECT, MORPH_HITMISS, add, bitwise_and, convertScaleAbs, fillPoly, findHomography, flip, getGaussianKernel, grabCut, imread,equalizeHist,IMREAD_GRAYSCALE,
                 createCLAHE,GaussianBlur, imwrite,medianBlur,Laplacian,Sobel,addWeighted, polylines, remap,subtract,filter2D,
                 CV_64F,pyrDown,pyrUp,resize,erode,dilate,getRotationMatrix2D,warpAffine,morphologyEx,MORPH_OPEN,MORPH_CLOSE,MORPH_GRADIENT,MORPH_TOPHAT,MORPH_BLACKHAT,cvtColor,Canny,threshold,DIST_L2,THRESH_BINARY_INV,THRESH_OTSU,
                 connectedComponents,distanceTransform,COLOR_BGR2GRAY,INTER_LINEAR,COLOR_BGR2HSV,
                 COLOR_BGR2YCrCb,Scharr,)#Prewitt,Roberts,COLOR_BGR2CMYK ,watershed
...
@app.route('/rotation_page')
def rotation_page():
    image_folder = app.config['OPERATIONS_FOLDER']
    image_files = os.listdir(image_folder)
    return render_template('rotation.html',image_files=image_files)


@app.route('/scaling_page')
def scaling_page():
    image_folder = app.config['OPERATIONS_FOLDER']
    image_files = os.listdir(image_folder)
    return render_template('scaling_page.html',image_files=image_files)
...
@app.route('/twenty', methods=['POST'])
def twenty():
    rotation_angle = 20  # Rotation angle in degrees
    operation_images = os.listdir(OPERATIONS_FOLDER)
    for img_name in operation_images:
        img_path = os.path.join(OPERATIONS_FOLDER, img_name)
        if os.path.isfile(img_path):
...
@app.route('/twentyfive', methods=['POST'])
def twentyfive():
    rotation_angle = 25  # Rotation angle in degrees
    operation_images = os.listdir(OPERATIONS_FOLDER)
    for img_name in operation_images:
        img_path = os.path.join(OPERATIONS_FOLDER, img_name)
        if os.path.isfile(img_path):
...
@app.route('/thirtyfive', methods=['POST'])
def thirtyfive():
...
@app.route('/forty', methods=['POST'])
def forty():
    rotation_angle = 40  # Rotation angle in degrees
    operation_images = os.listdir(OPERATIONS_FOLDER)
    for img_name in operation_images:
        img_path = os.path.join(OPERATIONS_FOLDER, img_name)
        if os.path.isfile(img_path):
...
@app.route('/fortyfive', methods=['POST'])
def fortyfive():
...
@app.route('/fifty', methods=['POST'])
def fifty():
    rotation_angle = 50  # Rotation angle in degrees
    operation_images = os.listdir(OPERATIONS_FOLDER)
    for img_name in operation_images:
        img_path = os.path.join(OPERATIONS_FOLDER, img_name)
        if os.path.isfile(img_path):
...
@app.route('/dot_25', methods=['POST'])
def dot_25():
    scale_factor = 0.25  # Scale factor for resizing the image
...
@app.route('/dot_75', methods=['POST'])
def dot_75():
    scale_factor = 0.75  # Scale factor for resizing the image
...
@app.route('/dot_1_25', methods=['POST'])
def dot_1_25():
    scale_factor = 1.25  # Scale factor for resizing the image
...
@app.route('/dot_1_5', methods=['POST'])
def dot_1_5():
    scale_factor = 1.5  # Scale factor for resizing the image
...
@app.route('/dot_1_75', methods=['POST'])
def dot_1_75():
    scale_factor = 1.75  # Scale factor for resizing the image
...
@app.route('/dot_2_0', methods=['POST'])
def dot_2_0():
    scale_factor = 2.0  # Scale factor for resizing the image
...
@app.route('/dot_2_25', methods=['POST'])
def dot_2_25():
    scale_factor = 2.25  # Scale factor for resizing the image
...
@app.route('/dot_2_5', methods=['POST'])
def dot_2_5():
    scale_factor = 2.5  # Scale factor for resizing the image
...
@app.route('/dot_2_75', methods=['POST'])
def dot_2_75():
    scale_factor = 2.75  # Scale factor for resizing the image
...
@app.route('/dot_3_0', methods=['POST'])
def dot_3_0():
    scale_factor = 3.0  # Scale factor for resizing the image
